...
def generate_plots(event=None):  # event parameter allows binding to key
    n = 150
    # Generate the Prime numbers sequence
    prime_numbers_sequence = prime_numbers(n)
    
    # Shuffle the Prime numbers randomly (no duplicates)
    np.random.shuffle(prime_numbers_sequence)
    
    # Generate a random array containing Prime numbers, ensuring no duplicates
    random_prime_numbers = np.random.choice(prime_numbers_sequence, size=n, replace=False)

    # Create a DataFrame from the random Prime numbers
    df = pd.DataFrame({'x': random_prime_numbers})
...
root = Tk()
root.title("Prime Numbers Plot Generator")
