...
# Function to generate Lucas Numbers
def lucas_numbers(n):
    lucas_seq = [2, 1]  # L0 = 2, L1 = 1
    for i in range(2, n):
        next_lucas = lucas_seq[-1] + lucas_seq[-2]
        lucas_seq.append(next_lucas)
    return lucas_seq

# Function to generate plots
def generate_plots(event=None):  # event parameter allows binding to key
    n = 150
    # Generate the Lucas Numbers sequence
    lucas_numbers_sequence = lucas_numbers(n)
    
    # Shuffle the Lucas Numbers randomly (no duplicates)
    np.random.shuffle(lucas_numbers_sequence)
    
    # Generate a random array containing Lucas Numbers, ensuring no duplicates
    random_lucas_numbers = np.random.choice(lucas_numbers_sequence, size=n, replace=False)

    # Create a DataFrame from the random Lucas Numbers
    df = pd.DataFrame({'x': random_lucas_numbers})
...
root = Tk()
root.title("Lucas Numbers Plot Generator")
