...
# Function to calculate Square numbers
def square_numbers(n):
    # Generate square numbers using the formula a_n = n^2
    sequence = [i**2 for i in range(1, n + 1)]
    return sequence

# Function to generate plots
def generate_plots(event=None):  # event parameter allows binding to key
    n = 150
    # Generate the Square numbers sequence
    square_numbers_sequence = square_numbers(n)
    
    # Shuffle the Square numbers randomly (no duplicates)
    np.random.shuffle(square_numbers_sequence)
    
    # Generate a random array containing Square numbers, ensuring no duplicates
    random_square_numbers = np.random.choice(square_numbers_sequence, size=n, replace=False)

    # Create a DataFrame from the random Square numbers
    df = pd.DataFrame({'x': random_square_numbers})
...
root = Tk()
root.title("Square Numbers Plot Generator")
